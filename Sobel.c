...
void read_pgm_file(char* dir, pgm* image) {
	...
	if (strcmp(image->version, "P2") == 0) {
		...
		#pragma omp omp parallel for num_threads(MAX_THREADS)
...
int convolution(pgm* image, int kernel[3][3], int row, int col) { // ham tinh tich chap
	int i, j, sum = 0;
	for (i = 0; i < 3; i++) {
		for (j = 0; j < 3; j++) {
...
void sobel_edge_detector(pgm* image, pgm* out_image) {
	int i, j, gx, gy;
	int hx[3][3] = { // ma tran mat na nhan
		{-1, 0, 1},
		{-2, 0, 2},
		{-1, 0, 1}
	};
	int hy[3][3] = { // ma tran mat na nhan
		{-1, -2, -1},
		{0, 0, 0},
		{1, 2, 1}
	};
	#pragma omp parallel for collapse(2) private(gx,gy) num_threads(MAX_THREADS)
	for (i = 1; i < image->height - 2; i++) {
		for (j = 1; j < image->width - 2; j++) {
			gx = convolution(image, hx, i, j); // dan xuat gx 
			gy = convolution(image, hy, i, j); // dan xuat gy
			out_image->imageData[i][j] = sqrt(gx*gx + gy*gy); // ket hop ket qua cua 2 dan xuat gx,gy
			// out_image->gx[i][j] = gx;
			// out_image->gy[i][j] = gy;
		}
	}
...
int main(int argc, char **argv)
{
	...
	write_pgm_file(&out_image, dir, out_image.imageData, ".ParallelG.pgm");
